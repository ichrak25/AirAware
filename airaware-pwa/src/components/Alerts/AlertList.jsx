...
/**
 * Single Alert Item - wrapped with forwardRef for AnimatePresence
 */
const AlertItem = forwardRef(function AlertItem({ alert, onResolve, onDismiss, onClick, compact = false }, ref) {
        ...
        >
            ...
            <div className="flex items-center gap-4 mt-3 text-sm text-slate-500">
                ...
                <div className="flex items-center gap-1">
                    ...
                    <span>{alert.sensorId || 'Unknown Sensor'}</span>
                </div>
            </div>
...
export default function AlertList({
    ...
    const sortedAlerts = [...filteredAlerts].sort((a, b) => {
        const dateA = new Date(a.triggeredAt || a.timestamp || a.createdAt || 0);
        const dateB = new Date(b.triggeredAt || b.timestamp || b.createdAt || 0);
        return dateB - dateA;
    });

    const displayAlerts = maxItems
        ? sortedAlerts.slice(0, maxItems)
        : sortedAlerts;
        ...
        <div className={compact ? 'space-y-2' : 'space-y-4'}>
            <AnimatePresence mode="popLayout">
                {displayAlerts.map((alert) => (
                    <AlertItem
                        key={alert.id || Math.random().toString(36).substr(2, 9)}
                        alert={alert}
                        onResolve={onResolve}
                        onDismiss={onDismiss}
