        ...
        >
            ...
            <div className="p-4 pb-0">
                ...
                <div className="flex items-center gap-1.5 text-sm text-slate-500 mt-2">
                    ...
                    <span>{sensor.location?.city || 'Unknown'}, {sensor.location?.country || 'Tunisia'}</span>
                </div>
            </div>
          ...
          <span className="text-slate-500">
            Model: {sensor.model || 'Raspberry Pi'}
          </span>
                    <span
                        className={`px-2 py-0.5 rounded-full font-medium ${status.bgClass}`}
...
export default function SensorsPage() {
    ...
    const loadData = async () => {
        ...
        try {
            console.log('[Sensors] Fetching real data from API...');

            // Fetch sensors
            let sensorsData = [];
            try {
                sensorsData = await sensorsAPI.getAll();
                console.log('[Sensors] Loaded:', sensorsData.length, 'sensors');
            } catch (e) {
                console.warn('[Sensors] Failed to fetch sensors:', e.message);
            }

            // Fetch readings
            let readingsData = [];
            try {
                readingsData = await readingsAPI.getLatest(100);
                console.log('[Sensors] Loaded:', readingsData.length, 'readings');
            } catch (e) {
                console.warn('[Sensors] Failed to fetch readings:', e.message);
            }

            const hasRealData = sensorsData.length > 0;

            if (hasRealData) {
                console.log('[Sensors] ✅ Using REAL DATA');
                ...
                console.log('[Sensors] ⚠️ No real data - using MOCK DATA');
                setUseMockData(true);
            ...
            console.error('[Sensors] Error loading data:', err);
    ...
    const filteredSensors = sensors.filter(sensor => {
        const sensorId = sensor.deviceId || sensor.sensorId || '';
        const description = sensor.description || sensor.name || '';
        ...
        return (
            <div className="flex items-center justify-center min-h-[60vh]">
                <div className="text-center">
                    ...
                    <p className="text-slate-600 dark:text-slate-400">Loading sensors...</p>
                </div>
            </div>
        );
    }
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-yellow-600 dark:text-yellow-400">
                                Connect your Raspberry Pi sensors to see real data
                            </p>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400">
                        {useMockData
                            ? 'Demo data - Connect Raspberry Pi for real sensors'
                ...
                <div className="glass-card p-4">
                    ...
                    <p className="text-2xl font-bold text-air-600">
                        {Math.round(
                            ...
                            Math.max(Object.values(currentReadings).filter(r => r?.pm25).length, 1)
                        ) || 0}
                    </p>
                </div>
            </div>
